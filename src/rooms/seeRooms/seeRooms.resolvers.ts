  ...
  Query: {
    seeRooms: async (_, __, { prisma, loggedInUser, handleCheckLogin }: Context): Promise<SeeRoomsResult> => {
      ...
      try {
        ...
        if (foundRooms.length === 0) {
          return { ok: false, message: "There are no existing chat rooms.", rooms: null };
        }

        return { ok: true, message: "You have successfully viewed all chat rooms.", rooms: foundRooms };
      } catch (error) {
        return { ok: false, message: `Failed to retrieve rooms: ${error.message}`, rooms: null };
      }
    },
  },
};
