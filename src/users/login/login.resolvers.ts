...
interface LoginResult extends CommonResult {
  token?: string;
  isLoggedIn: boolean; // isLoggedIn 속성 추가
}
  ...
  Mutation: {
    login: async (_, { username, password }: LoginArgs, { prisma }: Context): Promise<LoginResult> => {
      try {
        ...
        if (foundUser === null) {
          return { ok: false, message: "해당 계정이 존재하지 않습니다.", isLoggedIn: false };
        }

        const isPasswordMatch: boolean = await bcrypt.compare(password, foundUser.password);

        if (isPasswordMatch === false) {
          return { ok: false, message: "잘못된 비밀번호입니다.", isLoggedIn: false };
        }
        ...
        return { ok: true, message: "로그인에 성공하였습니다.", token, isLoggedIn: true };
      } catch (error) {
        console.log("로그인 에러");
        return { ok: false, message: "로그인에 실패하였습니다.", isLoggedIn: false };
      }
    },
  },
};
