...
const resolvers: Resolvers = {
  User: {
    ...
    following: async (parent: User, args, { prisma }: Context): Promise<User[] | null> => {
      try {
        const followingUsers: User[] = await prisma.user.findMany({
    ...
    followers: async (parent: User, args, { prisma }: Context): Promise<User[] | null> => {
      try {
        const followers: User[] = await prisma.user.findMany({
